...
Before production, create `production-plan.md` from `assets/production-plan.md` and confirm the following with the instructor in interactive modes. In Auto mode, select and record the strongest supported values internally, mark nondelegable decisions provisional or blocked, and proceed without an approval checkpoint:
...
For a spreadsheet, chart, or any artifact built from a dataset, apply `references/data-task-fit.md` first: execute the requested operation on the exact supplied file before producing the artifact, and record the schema actually read.

Read `references/visual-design.md` when layout, typography, color, hierarchy, diagrams, or visual identity materially affects the artifact. Follow supplied authoritative templates first; when none is supplied, resolve the palette by the mode rule in `references/visual-design.md` §3 — ask in Co-design and Guided, apply the example palette in Rapid and Auto — and never improvise colors. Record the applied palette, semantic roles, and contrast evidence.
