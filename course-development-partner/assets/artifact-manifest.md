...
Every teaching-ready artifact must also declare `Safety review` rather than leaving it blank: link the approved `safety-review.md` from `assets/safety-review.md` when students will encounter a physical hazard, or write `not required` when the artifact involves none. The declaration is required because this skill cannot detect hazards; the responsible safety owner, not the validator, makes the determination.
