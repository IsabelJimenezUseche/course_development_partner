...
`Dataset file` is a path relative to this file, and must stay inside the project — the exact file students will receive, not a description of it. A row whose dataset is absent records a claim nobody can re-check.
...
`Execution method` is `code` or `manual` — who or what performed the requested operation. It is not `validator`: `scripts/validate_data_task_record.py` checks structure and never draws the chart or computes the statistic, so naming it here would record an operation nobody performed. Use `manual` only when no execution capability is available, and say in `Result` what was inspected by hand.

`Execution evidence` points to what the execution produced — the chart, notebook, output file, or calculation record. Required when the method is `code`; for `manual` the `Result` text carries it. Like `Dataset file`, it must be a relative path inside the project: a portable project has to survive being copied, and an absolute path or `../` escape names a file the recipient will not have.
...
The columns above are the whole schema. Do not add project-specific columns: an extra column rides along unread by every check, and the validator reports one.
